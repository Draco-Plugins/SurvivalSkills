...
public class SpelunkerAbilityAsync extends BukkitRunnable {
    ...
    public SpelunkerAbilityAsync(SurvivalSkills plugin, Block playerBlock, int radius,
            HashMap<Block, ItemDisplay> currentGlowsRaw) {
    ...
    public void run() {
        ...
        for (Block block : blocks) {
            if (!ores.contains(block.getType()))
                continue; // Check for an ore

            // Check for the same world
            if (!block.getWorld().getEnvironment().equals(playerBlock.getWorld().getEnvironment()))
                continue;
            ...
            if (currentGlows.contains(block))
                currentGlows.remove(block); // If it already exists ignore it
            else
                glowsToAdd.add(block); // Otherwise add a glow block
        }
        // Any leftover blocks will be removed
        glowsToRemove = currentGlows;

        if (closestBlock != null)
            closestLocation = closestBlock;
    }
    ...
    public ArrayList<Block> getNearBlocks(Block block) {
        ...
        for (int i = 1; i <= radius * 2; i++)
            blocks.addAll(getBlockRowX(startBlock.getRelative(0, 0, i)));
        return blocks;
    }
    ...
    public ArrayList<Block> getBlockRowX(Block block) {
        ...
        for (int i = 1; i <= radius * 2; i++)
            blocks.addAll(getBlockColumn(block.getRelative(i, 0, 0)));
        return blocks;
    }
